      ...
      <dl className="-my-3 divide-y divide-gray-100 px-6 py-4 text-sm leading-6">
        <div className="flex justify-between gap-x-4 py-3">
          ...
          <dd className="text-gray-700">
            <time dateTime={application.lastInvoice.dateTime}>{application.lastInvoice.date}</time>
          </dd>
        </div>
        <div className="flex justify-between gap-x-4 py-3">
          <dt className="text-gray-500">Amount</dt>
          <dd className="flex items-start gap-x-2">
            <div className="font-medium text-gray-900">{application.lastInvoice.amount}</div>
            <div
              className={classNames(
                // statuses[application.lastInvoice.status],
                'rounded-md py-1 px-2 text-xs font-medium ring-1 ring-inset'
              )}
            >
              {application.lastInvoice.status}
            </div>
          </dd>
        </div>
      </dl>
